...
public class ControlBiomes extends Control implements ITooltip {
    ...
    public void getTooltip(final Tooltip list) {
        ...
        final int x = (int) (getRelativeMousePosition().x() / 16.0f);
        final int y = (int) (getRelativeMousePosition().y() / 16.0f);
        final int i = x + y * 8;
        if (i < tolerated.size()) {
            list.add(BiomeGenBase.getBiome(tolerated.get(i)).biomeName);
        }
    }
