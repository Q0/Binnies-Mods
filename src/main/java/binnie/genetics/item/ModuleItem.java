        ...
        for (final Item a : arr$) {
            for (final Item b : lyzers) {
                for (final Item c : lyzers) {
                    if (a != b && a != c && b != c) {
                        GameRegistry.addShapedRecipe(new ItemStack(Genetics.analyst), " b ", "fct", " d ", 'c', GeneticsItems.IntegratedCircuit.get(1), 'b', a, 't', b, 'f', c, 'd', new ItemStack(Items.diamond));
                    }
                }
            }
        }
                    ...
                    for (final Item c2 : dbs) {
                        for (final Item d : dbs) {
                            if (a2 != b2 && a2 != c2 && a2 != d && b2 != c2 && b2 != d && c2 != d) {
                                GameRegistry.addShapedRecipe(new ItemStack(Genetics.registry), " b ", "fct", " l ", 'c', GeneticsItems.IntegratedCircuit.get(1), 'b', a2, 't', b2, 'f', c2, 'l', d);
                            }
                        }
                    }
                }
