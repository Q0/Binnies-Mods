...
public class AnalystPageBehaviour extends ControlAnalystPage {
    public AnalystPageBehaviour(final IWidget parent, final IArea area, final IIndividual ind) {
        ...
        if (ind instanceof IButterfly) {
            ...
            new ControlTextCentered(this, y, "§oMetabolism: " + Binnie.Genetics.mothBreedingSystem.getAlleleName((IChromosomeType) EnumButterflyChromosome.METABOLISM, ind.getGenome().getActiveAllele((IChromosomeType) EnumButterflyChromosome.METABOLISM))).setColour(this.getColour());
            y += 20;
