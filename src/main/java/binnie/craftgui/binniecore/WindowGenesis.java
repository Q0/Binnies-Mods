    ...
    public void initialiseClient() {
        ...
        this.geneList = new ControlListBox<Gene>(this, one.x(), one.y(), one.w(), one.h(), 10.0f) {
            ...
            public IWidget createOption(final Gene value, final int y) {
                return new ControlGenesisOption(((ControlScrollableContent<ControlList<Gene>>) this).getContent(), value, y);
            }
        };
        this.geneOptions = new ControlListBox<Gene>(this, two.x(), two.y(), two.w(), two.h(), 10.0f) {
            @Override
            public IWidget createOption(final Gene value, final int y) {
                return new ControlTextOption<Object>(((ControlScrollableContent<ControlList<Gene>>) this).getContent(), value, y);
            }
        };
        tabSystems.addEventHandler(new EventValueChanged.Handler() {
            @Override
            public void onEvent(final EventValueChanged event) {
                WindowGenesis.this.root = event.getValue().getSpeciesRoot();
