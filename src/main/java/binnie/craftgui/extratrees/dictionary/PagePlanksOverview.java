...
public class PagePlanksOverview extends PageAbstract {
    ...
    public void onValueChanged(ItemStack species) {
        ...
        if (type != null) {
            ...
            if (fence != null) {
                (new ControlItemDisplay(this, (float) x, 48.0F)).setItemStack(fence);
                x += 22;
            }

            if (gate != null) {
                (new ControlItemDisplay(this, (float) x, 48.0F)).setItemStack(gate);
                x += 22;
            }

            if (door != null) {
                (new ControlItemDisplay(this, (float) x, 48.0F)).setItemStack(door);
                x = x + 22;
            }
        }

        ControlText controlDescription = new ControlText(this, new IArea(8.0F, 84.0F, this.getSize().x() - 16.0F, 0.0F), "", TextJustification.MiddleCenter);
        ControlText controlSignature = new ControlText(this, new IArea(8.0F, 84.0F, this.getSize().x() - 16.0F, 0.0F), "", TextJustification.BottomRight);
        String desc = "";
        if (type != null) {
